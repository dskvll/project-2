...
int main()
{
   ...
   if ((LionMouse = sem_open("LionMouse", O_CREAT, 0644, 1)) == SEM_FAILED) {
    perror("semaphore initilization");
    exit(1);
  }
  if (( Horse-Donkey = sem_open("Horse-Donkey", O_CREAT, 0644, 1)) == SEM_FAILED) {
    perror("semaphore initilization");
    exit(1);
  }
  if ((squirrel-penguin = sem_open("squirrel-penguin", O_CREAT, 0644, 1)) == SEM_FAILED) {
    perror("semaphore initilization");
    exit(1);
  }
  if ((Dog-seal = sem_open("Dog-seal", O_CREAT, 0644, 1)) == SEM_FAILED) {
    perror("semaphore initilization");
    exit(1);
  }
   ...
   case 2:  /*Dog code*/
                    ...
                    fflush(stdout);
                    sem_post(Horse-Donkey);
           break;

   case 3: /*Cat Code*/
                    printf("     Cat process with pid %d wants in.\n",pid);
                    fflush(stdout);
                    sem_wait(squirrel-penguin);
                    sem_wait(Horse-Donkey);
